...
//- (BOOL)loadTemplateDic:(NSDictionary *)dic {
...
//    NSArray *vList = [version componentsSeparatedByString:@"."];
//
//    short major = vList[0];
//    short minor = vList[1];
//    short patch = vList[2];
//    NSDictionary *subNode = nil;
//    while (subNode == nil) {
//        tag = [self readByte];
//        if (tag == VV_START_TAG) {
...
- (VVNodeCreater *)loadNodeDic:(NSDictionary *)subnodeDic
{
    VVNodeCreater *creater = [VVNodeCreater new];
    NSString *nodeID = subnodeDic[@"nodeName"];
    creater.nodeClassName = [VVNodeClassMapper classNameForType:nodeID];
    ...
    VVPropertySetter *setter = [[VVPropertySetter alloc] init];
